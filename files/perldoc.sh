...
do_perldoc() {
    ...
    GROFF="${GROFF:-groff -man}"
    TYPESETTER="${TYPESETTER:-}"
    GS="${GS:-gs}"
    GS_BATCH="${GS_BATCH:-$GS -dNOPAUSE -dQUIET -dBATCH}"
    GS_PDFWRITE="${GS_PDFWRITE:-$GS_BATCH -sDEVICE=pdfwrite -dPrinted=false}"
    # if gs could not find t42 fonts, specify that path 
    FONTPATH="${FONTPATH:-/usr/local/share/groff/site-font/devps}"
